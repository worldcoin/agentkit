...
export default function ClaimForm() {
	...
	const isLoading =
		state.step === 'fetching-challenge' || state.step === 'signing' || state.step === 'submitting'

	return (
		<div className="rounded-xl border border-border bg-surface-raised p-5">
			<div className="mb-4">
				<h2 className="text-sm font-semibold text-text-primary">Prove & Claim</h2>
				<p className="text-xs text-text-secondary mt-0.5">Sign a challenge to verify your agent is human-backed</p>
			</div>
			...
			<div className="space-y-3">
				{state.step !== 'disconnected' && (
					<div className="flex items-center gap-2 text-xs text-text-muted">
						...
						/>
						<div className="flex-1 h-px bg-border" />
						<StepIndicator active={state.step === 'submitting'} done={state.step === 'success'} label="Claim" />
					</div>
				)}
				...
				{state.step === 'disconnected' && (
					<p className="text-xs text-text-muted text-center">
						Your agent must be <a href="/register" className="text-primary hover:underline">registered</a> first.
					</p>
				)}
			</div>
		</div>
